...
def perform_blurring(ae, images_gt, experiment_name, slide_type):
    ...
    torch.save(reconstructed_images, f"reconstructed_blur_{experiment_name}.pt")
...
def perform_deblurring(ae, experiment_name, slide_type):
    ...
    right_encodings, right_rec = ae.encoder_decode(right_imgs)
...
def compute_metrics(experiment_name, slide_type, vnum=None, epoch=None, step=None):
    ...
    with torch.no_grad():
        try:
            ...
            ckpt_path = os.path.join(dir_path, "lightning_logs", f"version_{vnum}", "checkpoints",
        ...
        print(f"FFL (generated vs original) {ffl_gen_vs_orig}")
        print(f"FFL (generated vs reconstructed) {ffl_gen_vs_rec}")
        lds = compute_lds(latent_codes, interpolated_codes)
        print(f"LDS (original vs. interpolated latent codes) = {lds}")
        apd = compute_apd(latent_codes, interpolated_codes)
        print(f"APD (original vs. interpolated latent codes) = {apd}")
        psnr_gen_vs_rec = eval_psnr(generated_images, reconstructed_images)
        psnr_gen_vs_orig = eval_psnr(generated_images, images_gt)
        print(f"PSNR-blur-quality {psnr_gen_vs_orig}")
        print(f"PSNR-blur {psnr_gen_vs_rec}")
        print(f"Deblur PSNR-deblur-quality: {eval_psnr(sharp_generated, sharp_ground_truth)}")
        print(f"Deblur PSNR-deblur: {eval_psnr(sharp_generated, sharp_reconstructed)}")
...
if __name__ == "__main__":
    ...
    compute_metrics(args.experiment, args.slide_type, args.vnum, args.epoch, args.step)
    #
